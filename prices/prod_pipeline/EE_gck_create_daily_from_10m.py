...
def overlay_live_on_partial_day(
    bucket: Dict[str, Any],
    start: datetime,
    end: datetime,
    coin_row: Any,
) -> bool:
    live_price = sanitize_num(getattr(coin_row, "price_usd", None))
    live_lu = ensure_aware_utc(getattr(coin_row, "last_updated", None))
    if live_price is None or live_lu is None:
        return False
    if live_lu < start:
        return False

    # Keep the synthetic point inside the current partial day window.
    row_ts = live_lu if live_lu < end else (end - timedelta(seconds=1))
    if row_ts < start:
        return False

    live_point = SimpleNamespace(
        price_usd=live_price,
        market_cap=sanitize_num(getattr(coin_row, "market_cap", None)),
        volume_24h=sanitize_num(getattr(coin_row, "volume_24h", None)),
        market_cap_rank=getattr(coin_row, "market_cap_rank", None),
...
def main():
    ...
    for idx, c in enumerate(coins, 1):
        ...
        if partial_needed:
            d = today
            bucket = buckets.setdefault(d, make_bucket())
            overlay_live_on_partial_day(bucket, today_start, today_effective_end, c)
            price_count = bucket["price_count"]
            if price_count == 0:
                aggregate_existing(d)
                empty += 1
