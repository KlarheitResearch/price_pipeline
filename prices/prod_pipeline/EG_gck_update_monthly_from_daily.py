...
def main():
    ...
    finalize_prev = (not CURRENT_ONLY) and should_finalize_prev_month(today)
    ...
    print(
        f"[{now_str()}] Done. wrote={wrote} unchanged={unchanged} "
        f"errors={errors} current_only={CURRENT_ONLY}"
    )
    try:
        cluster.shutdown()
    except Exception as e:
